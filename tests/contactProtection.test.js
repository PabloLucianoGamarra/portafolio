  ...
  assert.equal(
    validateContact(
      contact({
        name: '李明',
        message:
          'Mi referencia es https://example.com y busco una web similar.',
      }),
    ),
    '',
  )
...
test('frena intentos repetidos y permite reintentar al cumplir el minuto', () => {
  ...
  assert.match(
    validateContact(contact(), previous, previous + 1000),
    /59 segundos/,
  )
  assert.equal(
    validateContact(contact(), previous, previous + CONTACT_COOLDOWN_MS),
    '',
  )
})
