...
export function validateContact(data, lastAttempt = 0, now = Date.now()) {
  ...
  if (name.length < 2 || name.length > 100 || message.length < 20 || message.length > 5000) {
  ...
  if (lastAttempt > 0 && now >= lastAttempt && now - lastAttempt < CONTACT_COOLDOWN_MS) {
    const seconds = Math.ceil((CONTACT_COOLDOWN_MS - (now - lastAttempt)) / 1000)
