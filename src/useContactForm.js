...
export function useContactForm() {
  ...
  async function submitContact(event) {
    ...
    if (!accessKey || !captchaToken) {
      setFormError(!accessKey ? 'El formulario todavía no está disponible. Podés usar el correo alternativo.' : 'Completá el CAPTCHA antes de enviar.')
      return
    }
    ...
    try {
      ...
      form.reset()
      setFormSuccess('¡Consulta enviada! Gracias por escribirme. Te responderé al correo que indicaste.')
    } catch {
      setFormError('No pudimos confirmar el envío. Conservamos tus datos. Esperá un minuto y completá nuevamente el CAPTCHA para reintentar, o usá el correo alternativo.')
    } finally {
      setCaptchaToken('')
  ...
  function handleCaptchaError() {
    setCaptchaToken('')
    setCaptchaError('No se pudo cargar o completar el CAPTCHA. Recargá la página o usá el correo alternativo.')
  }
